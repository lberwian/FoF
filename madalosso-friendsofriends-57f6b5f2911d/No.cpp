...
void No::relabel(vector<grupo*> *l, int g1, int g2) {
    ...
    ////    printf("\nTempo 1 : %d segundos e %d microssegundos", te.tv_sec - ts.tv_sec, 
    //abs(te.tv_usec-ts.tv_usec));
...
void No::add(vector<grupo*> *l, Corpo *novo, float xmax, float xmin, float ymax, float ymin, float zmax,
        float zmin, int *numeroGrupos, vector<string> *log) {
    int qx, qy, qz, quadro;
    float dx, dy, dz, dist;
    ...
    if (xmax - xmin < raio * 2 && ymax - ymin < raio * 2 && zmax - zmin < raio * 2) {
#if debug==true
        printf("CORPO ID = %d", novo->id);
        printf("\nRaio maior do que o cubo\n");
        stream.str("");
        stream << "\n\tCORPO ID: " << novo->id << "\n Raio Maior do que o cubo";
        log->push_back(stream.str());
#endif
        for (int j = 0; j < fronteira.size(); j++) {
            //            printf("\nComparando no novo com o %d da fronteira (ID=%d)", j, fronteira.at(j)->id);
            dist = ver(novo, fronteira.at(j));
#if debug==true
            stream.str("");
            stream << "\n\tComparando nó novo com o corpo ID: " << fronteira.at(j)->id;
            log->push_back(stream.str());
            stream.str("");
            stream << "\n\tDistancia : " << dist;
            log->push_back(stream.str());
#endif
            if (dist <= raio) {//exportar para função, código repetido sendo re-utilizado

#if debug==true
                printf("\n-f\t O nó pertence ao grupo %d", fronteira.at(j)->igru);
                stream.str("");
                stream << "\n\to nó pertence ao grupo " << fronteira.at(j)->igru;
                log->push_back(stream.str());
#endif
                if (novo->igru == -1) {
                    ...
                    log->push_back(stream.str());
#endif
                    novo->igru = fronteira.at(j)->igru;
                } else {
                    if (novo->igru != fronteira.at(j)->igru) {

#if debug==true
                        printf("\n\n\n-f\t **************pertence a 2 grupos"); //se o nó ja está em outro grupo,
                        //e se estiver
                        printf("\n-f\tPertence aos grupos : %d e %d", novo->igru, fronteira.at(j)->igru);
                        stream.str("");
                        stream << "\n\trolou RELABEL ";
                        log->push_back(stream.str());
#endif
                        relabel(l, novo->igru, fronteira.at(j)->igru);
                        if (novo->igru > fronteira.at(j)->igru) {
                            novo->igru = fronteira.at(j)->igru;
                        }
                    }
        ...
        if (novo->igru == -1) { //só cria novo grupo caso quando for criar o no, ele ainda não tiver
            ...
            log->push_back(stream.str());
#endif
            *numeroGrupos += 1; //incrementa variável estatica
#if debug==true
            printf("\n\t numeroGrupos : %d", *numeroGrupos);
#endif
            novo->igru = *numeroGrupos; //define igru como o novo grupo
            grupo *novo_grupo = new grupo(*numeroGrupos); //adiciona nova lista de nos(grupo novo)
            l->push_back(novo_grupo);
#if debug==true
            printf("\n Adicionado novo grupo, igru %d", *numeroGrupos);
#endif
        }
        ...
        return; //O PODEROSO RETURN! -------------------------------------- CONTEMPLE ------------
    }
#if debug==true
    //        printf("\n AREA DO CUBO");
    ...
    if (novo->x <= f_xmin || novo->x >= f_xmax || novo->y <= f_ymin || novo->y >= f_ymax
        ...
        stream.str("");
        stream << "\n\to novo não pertence a fronteira ";
        log->push_back(stream.str());
#endif
    }
    ...
    if (populado) {
        ...
        if (lim_max_x < lim_min_x) {
            float temp = lim_max_x;
            lim_max_x = lim_min_x;
            lim_min_x = temp;
        }
        if (lim_max_y < lim_min_y) {
            float temp = lim_max_y;
            lim_max_y = lim_min_y;
            lim_min_y = temp;
        }
        if (lim_max_z < lim_min_z) {
            float temp = lim_max_z;
            lim_max_z = lim_min_z;
            lim_min_z = temp;
        }
        ...
        if (c->x < no_velho->f_xmin || c->x > no_velho->f_xmax || c->y < no_velho->f_ymin ||
                c->y > no_velho->f_ymax || c->z < no_velho->f_zmin || c->z > no_velho->f_zmax) {
#if debug==true
            printf("\n-v\tO  no (id : %d )está na área de fronteira", c->id);
            stream.str("");
            stream << "\n-v\tO  no (id : " << c->id << ")está na área de fronteira";
            log->push_back(stream.str());
#endif
    ...
    if (filhos[quadro] != NULL) {
        if (novo->x < filhos[quadro]->f_xmin || novo->x > filhos[quadro]->f_xmax ||
            ...
            stream.str("");
            stream << "\n-f\tno filho está na zona de fronteira";
            log->push_back(stream.str());
#endif
        }
#if debug==true
    ...
    for (int i = 0; i < 8; i++) { //verifica todos corpos vizinhos ao cubo que irá add
        if (i != quadro && filhos[i] != NULL) {
            ...
            if (filhos[i]->populado) {
                ...
                stream.str("");
                stream << "\n-f\tDistancia entre nó e filhos[" << i << "](id = " << filhos[i]->c->id << ") = " << dist;
                log->push_back(stream.str());
#endif
                if (dist <= raio) {
#if debug==true
...
#if debug==true
                            printf("\n\n\n-f\t **************pertence a 2 grupos");
                            //se o nó ja está em outro grupo, e se estiver
                ...
                if (novo_fronteira || verF) {
                    ...
                    for (int j = 0; j < filhos[i]->fronteira.size(); j++) {
                        ...
                        stream.str("");
                        stream << "\n-f\tVerifica distancia com o no " << f->id;
                        log->push_back(stream.str());
#endif
                        dist = ver(f, novo);
#if debug==true
